...
const Chat = () => {
  ...
  const [privateMessagesData, setPrivateMessagesData] = useState<MessagesArray>(
    []
  ); // messages du channel privé actif
  ...
  useEffect(() => {
    // Handle events : 'fetch channels', 'fetch mp'
    ...
    socket?.on("fetch channels", (data: channelModel[]) => {
      const tab: channelModelList = !Array.isArray(data) ? Array.from(data) : data;
      for (let index = 0; index < tab.length; index++) {
        if (
          tab[index].members.some(
            (member) => member.name === userInfos.userName.userName
          )
        )
          tab[index].isMember = true;
        else tab[index].isMember = false;
      }
      setChannels(tab);
    ...
    socket?.on("duel response", (data: any) => {
      if (data.response === true)
        setTextToDisplay(`Request accepted. Creating lobby with ${data.name}`);
  ...
  useEffect(() => {
    // handle events : 'fetch message', 'update requests'
    console.log("activeChannel", activeChannel);
    if (activeChannel && socket) {
      socket.emit("get messages", activeChannel);
      socket?.emit("get role", activeChannel);
    } else if (activePrivateChannel && socket)
      socket.emit("get messages", activePrivateChannel);
  ...
  }, [activeChannel, activePrivateChannel, socket, channels]);
  ...
  useEffect(() => {
    // recuperer le role d'utilisateur
    if (activeChannel && socket) {
  ...
  const handleChannelClick = (channelId: number) => {
    ...
    const channel: channelModel | undefined = channels.find(
    ...
    if (channel && channel.isProtected) {
      ...
      console.log("activ channelId:", channelId);
  ...
  const handleClickMembers = (channel: channelModel, action: string) => {
    ...
    setChannels((prevState: channelModelList) => {
      const foundChannelIndex = prevState.findIndex(
        (value: channelModel) => value.id === channel.id
      );

      if (foundChannelIndex === -1) {
        if (action === "join") {
          return [...prevState, { ...channel, isMember: true }];
        }

        return prevState;
      }

      const foundChannel = { ...prevState[foundChannelIndex] };
      foundChannel.isMember = action === "join";

      const updatedChannels = [...prevState];
      updatedChannels[foundChannelIndex] = foundChannel;
  ...
  const handlePrivMsgClick = (channelId: number) => {
    const channel: channelModel | undefined = priv_msgs.find(
      (c: any) => c.id === channelId
    );
    if (!channel) return;
    setMpInfos(channel);
  ...
  const handleUserClick = async (userName: string, userID: number) => {
    if (userInfos.userName.userName == userName) return;
  ...
  const handlePasswordSubmit = (password: string) => {
    ...
    console.log(
      "🚀 ~ file: Chat.tsx:416 ~ handlePasswordSubmit ~ tempActiveChannel:",
      tempActiveChannel
    );
    if (channel && socket) {
    ...
    <div className="chat">
      <div className="chan_privmsg_container">
        <div className="channel_part" onClick={closeUserPopup}>
          ...
          <div className="channel_top_div">
            ...
            <h1 className="createchan" onClick={createChannel}>
              +
            </h1>
            {isPopupOpen && (
              <ChannelNamePopup
          ...
          <div className="channel_div_container">
            {channels.map((channel: channelModel, index: number) => {
              // console.log("PasswordNeeded, !channel.isMember", PasswordNeeded, !channel.isMember)
              return (
                <div
                  key={index}
                  className="channel_div"
                >
                  {PasswordNeeded && !channel.isMember && (
                    <PrivateChanPopup
                      onClose={PasswordDone}
                  ...
                  <div className="channel_content">
                    <div
                      onClick={() => {
                        channel.isMember && handleChannelClick(channel.id);
                      }}
                    >
                      <h1
                        ...
                        onClick={() => {
                          channel.members.some(
                            (member) =>
                              member.name === userInfos.userName.userName
                    ...
                    {channel.isMember ? (
                      <div
                        className="leaveChannelButton"
                        onClick={(e) => handleClickMembers(channel, "leave")}
                      ></div>
                    ) : (
                      <div
                        className="joinChannelButton"
                        onClick={(e) => handleClickMembers(channel, "join")}
                      ></div>
                    )}
                  </div>
                </div>
        ...
        <div className="priv_msg_part">
          <h1 className="h1_channel">
            #MP List({priv_msgs ? priv_msgs.length : 0})
          </h1>
          <div className="private_users_container">
            {priv_msgs.map((privateChannel: channelModel, index: number) => (
              <div
                className="channel_div privmsg"
                key={index}
                onClick={() => handlePrivMsgClick(privateChannel.id)}
              >
                <div className="channel_content">
      ...
      <div className="main_part">
        {activeChannel ? (
          <div className="message_list">
            <h1 className="channel_title active_channel_title">
              #{channelName}
            </h1>
            <ul>
              {activeChannel &&
                ...
                messagesData.map((message: MessageData, index: number) => {
                  const textColor = check_user(message.email)
                    ? "#8f35de"
                    : "#275ec4"; //"#8f35de" : "#275ec4"
                  return (
                    <li key={index}>
                      <div
                        className={`message_bubble ${
                          message.name !== userInfos.userName.userName
                            ? "user"
                            : "not_user"
                        }`}
                        style={{
                      ...
                      >
                        {selectedUser !== userInfos.userName.userName ? (
                          <span
                            ...
                            onClick={() =>
                              handleUserClick(message.name, message.id)
                            }
                    ...
                    <li key={index}>
                      ...
                      >
                        <span
                          ...
                          onClick={() =>
                            handleUserClick(message.name, message.id)
                          } // Gérer le clic sur le nom de l'utilisateur
                        >
                          <strong>{message.name} </strong>
                        </span>
                        <span style={{ color: "#c0b9c7" }}>
      ...
      <div
        className={`user_popup ${
          isUserPopupVisible && selectedUser ? "visible" : ""
        }`}
      >
